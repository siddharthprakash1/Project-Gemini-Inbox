...
class GmailAssistant:
    ...
    def _initialize_agent(self):
        ...
        try:
            if not self.tools: # Check if tools are available before initializing agent
                print("Agent initialization skipped because no tools are available (e.g., Serper API key missing).")
                return
            print("Initializing agent with tools...")
            self.agent = initialize_agent(
                self.tools,
                self.llm,
                agent=AgentType.ZERO_SHOT_REACT_DESCRIPTION, # Or use a more advanced agent type
                verbose=True # Keep verbose for debugging
            )
            print("Agent initialized successfully.")
        except Exception as e:
            print(f"❌ Agent initialization error: {e}")
            self.agent = None # Handle case where agent fails to initialize
    ...
    def process_email(self, message_id):
        ...
        try:
            ...
            try:
                ...
                if self.agent:
                    analysis_result = self.agent.run(
                        f"Analyze this email for priority, category, sentiment, summary, key points, required action, content value, and engagement level. Return a JSON object with these keys. Email From: {sender}, Subject: {subject}, Body: {body}"
                    )
                    analysis = {'text': analysis_result} # Wrap agent output in 'text' for consistent parsing
                ...
                if self.agent:
                    response_result = self.agent.run(
                        f"Based on the analysis: {json.dumps(analysis_json)}, draft an engaging and detailed email response to: From: {sender}, Subject: {subject}, Body: {body}. Return a JSON object for the response including 'response_text', 'tone', 'follow_up_needed', 'key_response_points', and 'engagement_type'."
                    )
                    response = {'text': response_result} # Wrap agent output in 'text' for consistent parsing
